        ...
        {menuItems.map((item) => {
          ...
          return (
            <Link
              ...
              `}
            >
              <Icon size={18} className={isActive ? "text-orange-800" : "text-gray-500"} />
              <span>{item.name}</span>
            </Link>
          );
        })}

        <button className="mt-4 flex items-center justify-center gap-2 text-red-600 font-medium bg-red-50 px-3 py-2 rounded-lg hover:bg-red-100 transition-all duration-200 w-full md:w-full">
          Log out
          <FiLogOut size={18} />
        </button>
